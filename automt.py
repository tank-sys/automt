...
def helppage():
    print(color.BOLD+"Penggunaan :"+color.END)
    print(" "+color.BOLD+"python automt.py"+color.END)
    print(" Default. Alamat ip akan membaca seluruh alamat yang ada di file semuaip.")
    print(" "+color.BOLD+"python automt.py withlog"+color.END)
    print(" Membuat file log : python automt.py ")
    print(" "+color.BOLD+"python automt.py ip {ipaddress}"+color.END)
...
def main():
    ...
    if len(sys.argv)==2:
        if sys.argv[1] == ("withlog"):
            ...
            print(color.RED+"error : withlog atau ip {ipaddress}"+color.END)
    elif len(sys.argv)==3:
        if sys.argv[1] == ("ip"):
            verifdevice(sys.argv[2])
        else:
            print(color.RED+"cara : python automt.py ip {ipaddress}"+color.END)
    elif len(sys.argv)==1:
        md()
    else:
        print(color.RED+"error gak jelas : coba dibaca hasilnya python automt.py -h "+color.END)
...
def md():
    if os.path.exists(fld):
        ...
        for device in ad:
            ...
            ldc = len(dd)
            print("+----------------------------------+")
            print("+ "+str(i)+". IP Device : "+color.CYAN+dd[0]+color.END)
            print("+----------------------------------+")
            if ldc != 1:
